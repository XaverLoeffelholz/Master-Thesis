...
public class Face : MonoBehaviour {
	...
	public void ReplaceFaceOnOtherFace(Face other, Vector3 distance, bool reverseOrder){
		if (reverseOrder) {
			for (int i = 0; i < vertexBundles.Length; i++) {
				vertexBundles [i].coordinates = other.vertexBundles [i].coordinates + distance;
			}
		} else {
			for (int i = 0; i < vertexBundles.Length; i++) {
				vertexBundles [i].coordinates = other.vertexBundles [vertexBundles.Length-i-1].coordinates + distance;
			}
		}
